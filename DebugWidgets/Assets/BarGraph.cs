...
public class BarGraph : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            signedDisplay = true;
            BarTransform.localPosition = new Vector3(width / 2, BarTransform.localPosition.y, 0);
        }
    ...
    public void SetWidthPercent(float value, Color c)
    {
        ...
        if (signedDisplay)
        {
            BarTransform.localScale = new Vector3(value / (2*Max), 1, 1);
        } else
        {
            BarTransform.localScale = new Vector3(value / Max, 1, 1);
        }
    }
    ...
    public static BarGraph Find(string name, Vector2 position, float min, float max)
    {
        ...
        if (BarGraphTable.TryGetValue(name, out var bargraph))
        {
            return bargraph;
        }
        //
        ...
        // "don't rotate it".
        GameObject go = Instantiate(Prefab, position, Quaternion.identity);  // Change null to a call to Instantiate
